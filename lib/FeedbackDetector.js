...
class FeedbackDetector {
  ...
  async start(arg = {}) {
    ...
    // Stream -- stop old tracks before replacing to prevent mic leak
    if (stream) {
    ...
    // GainNode: source -> gainNode -> analyser (passive, never to destination)
    if (!this._gainNode) {
  ...
  _allocateBuffers() {
    ...
    // Only reset noise floor if no manual override is active
    ...
    for (let i = start; i <= end; i++) {
      ...
      if (valid) {
        ...
        if (hold[i] >= this._sustainMs) {
          ...
          // Fire callback on initial detection and periodically (~500ms) while active
          if (!this._lastUpdateTs) this._lastUpdateTs = new Float64Array(this._binCount);
  ...
  _updateNoiseFloorDb(dt) {
    // If user has set a manual override, skip entirely (no fill, no median)
